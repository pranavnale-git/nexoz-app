...
const nexozData = {
  ...
  services: [
    {
      title: "Launchpad Website Creation",
      price: "From ₹7,999",
      features: ["Custom Landing Page", "Integrated Forms/Chatbot", "SEO Optimized Launch"]
    },
    {
...
const Navbar = ({ darkMode, setDarkMode }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 backdrop-blur-lg border-b transition-all duration-300 ${isHeaderVisible ? 'translate-y-0' : '-translate-y-full'} ${darkMode ? 'border-white/10 bg-black/50' : 'border-black/5 bg-white/70'}`}>
      ...
      {/* MOBILE MENU FIX: Ensure text is visible regardless of theme */}
      <div className={`md:hidden absolute w-full transition-all duration-300 ease-in-out overflow-hidden ${isMobileMenuOpen ? 'max-h-screen opacity-100 border-b border-white/10 bg-black/90' : 'max-h-0 opacity-0'}`}>
        <div className={`flex flex-col p-6 gap-6 text-center font-bold text-lg ${darkMode ? 'text-white' : 'text-slate-900'}`}>
          {nexozData.nav.map((item) => (
            <button key={item} onClick={() => handleScrollTo(item.toLowerCase())} className={`hover:text-blue-500 transition-colors ${darkMode ? 'text-white' : 'text-slate-200'}`}>{item}</button>
          ))}
        </div>
      </div>
    </nav>
      ...
      <section id="home" className="relative pt-32 pb-20 px-6 min-h-[85vh] flex flex-col justify-center items-center text-center overflow-hidden">
        <FadeInSection>
          ...
          <h1 className={`text-5xl sm:text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter leading-tight mb-8 max-w-6xl mx-auto drop-shadow-2xl bg-clip-text text-transparent bg-gradient-to-b ${darkMode ? 'from-white to-white/60' : 'from-slate-900 to-slate-600'}`}>
             {/* Text interaction is fixed for both themes */}
      ...
      <section id="compatibility" className="py-24 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <FadeInSection>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 mx-auto w-full"> 
